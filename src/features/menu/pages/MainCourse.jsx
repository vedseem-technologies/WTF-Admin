...
import { Search, ClipboardList, Check, X, Utensils } from "lucide-react";
...
const MainCourse = () => {
  ...
  // Filter items for Main Course category (category ID = 2)
  const mainCourseItems = menuItems.filter((item) => item.category === 2);

  // Filter by search term
  const filteredItems = mainCourseItems.filter((item) =>
    ...
    <div className="p-6 lg:h-[calc(100vh-100px)] flex flex-col">
      <div className="mb-6 flex-shrink-0">
        <h2 className="text-2xl font-bold text-secondary flex items-center">
          <Utensils className="inline mr-2 text-primary" size={24} /> Main
          Course Items
        </h2>
        <p className="text-sm text-gray-500">
          Browse and add main course items to your selection
        </p>
      </div>
