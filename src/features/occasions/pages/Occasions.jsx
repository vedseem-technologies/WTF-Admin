    ...
    <div className="p-6">
        ...
        <div className="flex items-center gap-2 ml-auto">
          {["all", "active", "inactive"].map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-all capitalize ${filter === f ? "bg-primary text-white shadow-sm" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
            >
      ...
      {loadingOccasions && occasions.length === 0 ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
            {occasions.map((occasion) => (
              <div
                key={occasion._id}
                className={`bg-white rounded-xl border border-border shadow-sm hover:shadow-md transition-all overflow-hidden ${!occasion.active ? "opacity-60" : ""}`}
              >
        ...
        <div className="flex flex-col items-center justify-center py-20 text-gray-400">
          <PartyPopper size={48} className="mb-4 text-gray-400" />
          <h3 className="text-lg font-semibold mb-1">No occasions found</h3>
          <p className="text-sm">
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Show this occasion on the frontend
                </p>
              </div>
              <Toggle
